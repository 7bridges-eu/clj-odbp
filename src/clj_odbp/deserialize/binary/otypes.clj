...
(defn bool-orient-type
  ...
  [buffer position]
  (let [lenght (v/varint-signed-long (take-n-bytes buffer 1))
        b (take-n-bytes buffer lenght)]
    (apply str (map char b))))

(defn integer-orient-type
  "Read a integer from the stream"
  [buffer position]
  (let [b (to-bytearray (drop position buffer))]
    (int (v/varint-signed-long (take-n-bytes b 1)))))
...
(defn string-orient-type
  ...
  [buffer position]
  (let [b (to-bytearray (drop position buffer))
        size (v/varint-signed-long (take-n-bytes b 1))]
    (apply str (map char (take-n-bytes b size)))))
